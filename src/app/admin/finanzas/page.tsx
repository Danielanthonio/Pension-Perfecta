...
// Quién entra aquí:
//   · finanzas        → el módulo completo, y es lo ÚNICO que ve de la app
//   · director/admin  → el módulo completo (si está en la lista de acceso)
...
// en la base. Las tablas de comisiones solo tienen política de SELECT para quien
// pasa `fin_is_direccion()` y todas las RPC llaman a `fin_require_direccion()`
// por dentro, así que un usuario que llegara aquí a mano no vería un solo peso.
const ROLES_CON_FINANZAS = ["director", "finanzas"];
...
function FinanzasGate() {
  ...
  if (!ROLES_CON_FINANZAS.includes(user.role)) {
    return (
